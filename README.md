...
```mermaid
graph TB
    A[사용자] --> B[React/Next.js Frontend]
    B --> C[Spring Security + JWT]
    C --> D[Spring Boot Backend]
    D --> E[JPA/Hibernate]
    E --> F[MySQL Database]
    
    B --> G[금융 서비스<br/>예적금/대출/주식]
    B --> H[부동산 서비스<br/>청약/맵/캘린더]
    B --> I[취미 서비스<br/>식물/동물/소모임]
    B --> J[커뮤니티 서비스<br/>게시판/파티]
    
    D --> K[Finance Service]
    D --> L[Housing Service]
    D --> M[Hobby Service]
    D --> N[Community Service]
    
    F --> O[Finance Tables]
    F --> P[Housing Tables]
    F --> Q[Hobby Tables]
    F --> R[Community Tables]
...
```mermaid
graph LR
    A[공공데이터 포털<br/>부동산 청약 정보] --> D[Spring Boot Backend]
    B[카카오맵 API<br/>지도 서비스] --> D
    C[금융감독원 API<br/>금융 상품 정보] --> D
    
    D --> E[REST API Controller]
    E --> F[React Frontend]
...
### 모듈별 역할 분담
- **dahonjasanda-front**: 사용자 인터페이스 및 사용자 경험 담당
- **dahonjasanda-back**: 비즈니스 로직, API 서버, 인증/인가 처리
- **dahonjasanda-db**: 데이터 모델 정의 및 데이터베이스 스키마 관리
