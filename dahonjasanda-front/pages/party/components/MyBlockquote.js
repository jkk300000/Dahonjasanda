const MyBlockquote = (props) => {
  ...
  if (thumbSrc) {
    footer = <footer className={`d-flex text-start align-items-center${alignmentFooter}`}>
      ...
      <div className='ps-3'>
        <h6 className={`fs-base mb-0${light}`} style={{ backgroundColor: 'whitesmoke' }}>{name}</h6>
    ...
    footer = <footer className={`blockquote-footer${light}`} style={{ backgroundColor: 'whitesmoke' }}>{name}</footer>
  }

  // Render markup
  return (
    <blockquote className={`blockquote${extraClass}${alignment}`} style={{ borderRadius: '10px', ...style}} >
      <div style={{ height: '120px'}} className={`pb-1 mb-3${light}`}>{children}</div>
      {footer}
    </blockquote>
  )
}
